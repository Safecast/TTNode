...
#ifdef FLOWTRACE
        DEBUG_PRINTF("(I'll send %s in a moment.)\n", comment);
#endif
    } else {
        deferred_transmit = false;
        lora_send(deferred_transmit_buffer);
...
void lora_process() {
    ...
    case COMM_LORA_GETVERRPL: {
        ...
        // Init into the appropriate mode
        STORAGE *s = storage();
    ...
    case COMM_LORA_INITCOMPLETED: {
        ...
        // Note that if we've successfully entered LoRaWAN mode,
        // we need to block the UART from ever switching away.  This is
        // because LoRaWAN doesn't permit more than 12 "join" requests per day,
        // which makes oneshot mode non-viable.
        if (LoRaWAN_mode)
            comm_disable_oneshot_mode();
    ...
    case COMM_LORA_LORAREQ: {
        DEBUG_PRINTF("LPWAN initializing (Lora)\n");
    ...
    case COMM_LORA_LORAWANREQ: {
        DEBUG_PRINTF("LPWAN initializing (LoRaWAN)\n");
        lorafpRegionCommandNumber = 0;
...
#define delay_interval_ms 100
        ...
        comm_cmdbuf_set_state(&fromLora, COMM_STATE_IDLE);
        if (send_to_service("relay", toRelayBuffer, toRelayBufferLength, REPLY_NONE))
